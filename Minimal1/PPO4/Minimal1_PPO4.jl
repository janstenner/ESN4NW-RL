...
function initialize_setup(;use_random_init = false)
    ...
    global env = GeneralEnv(do_step = do_step, 
        ...
        global agent = create_agent_ppo4(
                ...
                update_freq_no_explore = update_freq_no_exploration,
...
function render_run(; plot_optimal = false, steps = 6000, show_training_episode = false, show_σ = false, exploration = false, return_plot = false, gae = false, plot_values = false, plot_critic2 = false, critic2_diagnostics = false)
    ...
    while !env.done
        ...
        if exploration && !agent.policy.exploration_triggered
            if is_inside(agent.policy.trigger_set, env.state)
                println("trigger at time $(env.time/60)")
                agent.policy.exploration_triggered = true
            end
        end
    ...
    agent.policy.exploration_triggered = false
    # if use_best
