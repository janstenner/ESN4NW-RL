...
function sample_zone_centers()
    ...
    for seg in 1:segments
        # rejection sample three centers with pairwise min spacing
        tries = 0
        while true
            tries += 1
            vals = rand(Float32, n_zones) .* (high - low) .+ low
            ok = all(abs(vals[i] - vals[j]) >= min_gap for i in 1:n_zones for j in i+1:n_zones)
            if ok
                centers[:, seg] = vals
                break
            end
...
function sample_zone_potential_rewards()
    biases = (0.0f0, 1.0f0, -1.0f0) .* bias_strength
...
try
    global validation_set = FileIO.load("./TestEnvironments/Test3/validation_set.jld2", "set")
catch
    global validation_set = generate_validation_set()
    FileIO.save("./TestEnvironments/Test3/validation_set.jld2", "set", validation_set)
end
